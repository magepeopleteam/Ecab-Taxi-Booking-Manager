		...
		class MPTBM_Woocommerce {
			...
			public function before_calculate_totals( $cart_object ) {
				foreach ( $cart_object->cart_contents as $value ) {
					$ttbm_id = array_key_exists( 'mptbm_id', $value ) ? $value['mptbm_id'] : 0;
					if ( get_post_type( $ttbm_id ) == MPTBM_Function::get_cpt_name() ) {
			...
			public function cart_item_thumbnail( $thumbnail, $cart_item ) {
				$ttbm_id = array_key_exists( 'mptbm_id', $cart_item ) ? $cart_item['mptbm_id'] : 0;
				if ( get_post_type( $ttbm_id ) == MPTBM_Function::get_cpt_name() ) {
					$thumbnail = '<div class="bg_image_area" data-href="' . get_the_permalink( $ttbm_id ) . '"><div data-bg-image="' . MPTBM_Function::get_image_url( $ttbm_id ) . '"></div></div>';
				}
				return $thumbnail;
			}
				...
				if ( get_post_type( $post_id ) == MPTBM_Function::get_cpt_name() ) {
					...
					if ( sizeof( $extra_service ) > 0 ) {
						foreach ( $extra_service as $service ) {
							...
							$item->add_meta_data( esc_html__( 'Price : ', 'mptbm_plugin' ), ' ( ' . MPTBM_Function::wc_price( $post_id, $service['service_price'] ) . ' x 1) = ' . MPTBM_Function::wc_price( $post_id, ( $service['service_price'] * 1 ) ) );
						}
					}
					...
					<?php if ( sizeof( $extra_service ) > 0 ) { ?>
						...
						<?php foreach ( $extra_service as $service ) { ?>
							<div class="dLayout_xs">
								<ul class="cart_list">
									<li>
										<h6><?php esc_html_e( 'Services Name : ', 'mptbm_plugin' ); ?></h6>&nbsp;
										<span><?php echo esc_html( $service['service_name'] ); ?></span>
									</li>
									<li>
										<h6><?php esc_html_e( 'Price : ', 'mptbm_plugin' ); ?></h6>&nbsp;
										<span><?php echo ' ( ' . MPTBM_Function::wc_price( $post_id, $service['service_price'] ) . ' x 1 ) = ' . MPTBM_Function::wc_price( $post_id, ( $service['service_price'] * 1 ) ); ?></span>
									</li>
								</ul>
							</div>
						<?php } ?>
