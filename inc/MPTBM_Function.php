	...
	if ( ! class_exists( 'MPTBM_Function' ) ) {
		class MPTBM_Function {
			...
			public static function get_date( $tour_id, $expire = '' ) {
				$tour_date   = [];
				$travel_type = MPTBM_Function::get_travel_type( $tour_id );
				$now         = strtotime( current_time( 'Y-m-d H:i:s' ) );
				if ( $travel_type == 'particular' ) {
					$particular_dates = MPTBM_Function::get_post_info( $tour_id, 'ttbm_particular_dates', array() );
					if ( sizeof( $particular_dates ) > 0 ) {
						foreach ( $particular_dates as $date ) {
							$time      = $date['ttbm_particular_start_time'] ?: '23.59.59';
							$full_date = MPTBM_Function::reduce_stop_sale_hours( $date['ttbm_particular_start_date'] . ' ' . $time );
					...
					$start_date    = MPTBM_Function::get_post_info( $tour_id, 'ttbm_travel_repeated_start_date' );
					$start_date    = $start_date ? date( 'Y-m-d', strtotime( $start_date ) ) : '';
					$end_date      = MPTBM_Function::get_post_info( $tour_id, 'ttbm_travel_repeated_end_date' );
					$end_date      = $end_date ? date( 'Y-m-d', strtotime( $end_date ) ) : '';
					$off_days      = MPTBM_Function::get_post_info( $tour_id, 'mep_ticket_offdays', array() );
					$all_off_dates = MPTBM_Function::get_post_info( $tour_id, 'mep_ticket_off_dates', array() );
					$off_dates     = array();
					...
					if ( $start_date == $end_date ) {
						...
						$interval  = MPTBM_Function::get_post_info( $tour_id, 'ttbm_travel_repeated_after', 1 );
					...
					if ( $date ) {
						...
						if ( $expire || ( $now <= strtotime( $full_date ) && $tour_status == 'active' ) ) {
							...
							$tour_date['checkout_date'] = MPTBM_Function::get_post_info( $tour_id, 'ttbm_travel_end_date' );
						}
					}
				}
			...
			public static function get_date_by_time_check( $tour_id, $date, $expire ) {
				$tour_date = '';
				$now       = strtotime( current_time( 'Y-m-d H:i:s' ) );
				$times     = MPTBM_Function::get_time( $tour_id, $date, $expire );
				if ( is_array( $times ) && sizeof( $times ) > 0 ) {
					foreach ( $times as $time ) {
						$full_date = $time['time'] ? $date . ' ' . $time['time'] : $date . ' ' . '23.59.59';
						$full_date = MPTBM_Function::reduce_stop_sale_hours( $full_date );
						if ( $expire || $now <= strtotime( $full_date ) ) {
							$tour_date = $date;
						}
					}
				} else {
					$full_date = MPTBM_Function::reduce_stop_sale_hours( $date . ' ' . '23.59.59' );
					if ( $expire || $now <= strtotime( $full_date ) ) {
						$tour_date = $date;
					}
				}
			...
			public static function get_total_sold( $tour_id, $tour_date = '', $type = '', $hotel_id = '' ): int {
				$tour_date  = $tour_date ?: MPTBM_Function::get_post_info( $tour_id, 'ttbm_upcoming_date' );
			...
			public static function get_total_service_sold( $tour_id, $tour_date, $type = '' ): int {
				$sold_query = MPTBM_Query::query_all_service_sold( $tour_id, $tour_date, $type );
				return $sold_query->post_count;
			}
			...
			public static function tour_type() {
				$type = array(
					'general' => __( 'General Tour', 'mptbm_plugin' ),
					'hotel'   => __( 'Hotel Base Tour', 'mptbm_plugin' )
				);
			...
			public static function travel_type_array(): array {
				return array(
					'fixed'      => __( 'Fixed Dates', 'mptbm_plugin' ),
					'particular' => __( 'Particular Dates', 'mptbm_plugin' ),
					'repeated'   => __( 'Repeated Dates', 'mptbm_plugin' )
				);
			}
			public static function get_tour_status( $tour_id, $status = 'active' ) {
				$tour_type = self::get_tour_type( $tour_id );
				$date_type = MPTBM_Function::get_travel_type( $tour_id );
			...
			public static function get_all_location(): array {
				...
				$arr       = array(
					'' => esc_html__( 'Please Select a Location', 'mptbm_plugin' )
				);
